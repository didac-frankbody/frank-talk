...
test("home page loads with frank talk branding", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("main").getByRole("img", { name: "frank talk" }),
  ).toBeVisible();
});
...
test("invite requires age and legal consent before opening frank talk", async ({
  page,
}) => {
  ...
  const acceptInvite = page.getByRole("button", {
    name: "Accept invite in frank talk",
  });
  ...
  await page
    .locator("label")
    .filter({
      hasText: "I agree to the frank talk Terms of Service and Privacy Policy.",
    })
