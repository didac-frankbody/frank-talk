...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "[Buzz event]" marker (the harness wire format), there is no
  // event content to surface, so userText is empty and the displayed title
  // falls through to the default, "frank talk event".
  ...
  assert.equal(result.userTitle, "frank talk event");
...
test("parsePromptText defaults the displayed title when no kind is present", () => {
