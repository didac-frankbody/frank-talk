...
function buildMockConfigSurface(pubkey: string): {
  ...
  // panel witnesses distinct provenance labels in one frame: "Set in frank talk",
...
function resetMockPersonas(config?: E2eConfig) {
  ...
  const builtInPersonas = [
    ...
    },
    {
      ...
      display_name: "Rosie",
      avatar_url: null,
      system_prompt: "You are Rosie.",
    },
    {
      id: "builtin:bumble",
      display_name: "Clay",
      avatar_url: null,
      system_prompt: "You are Clay.",
    },
  ];
...
const MOCK_PROJECT_SEEDS = [
  {
    ...
    name: "buzz",
    description:
      "Relay, desktop, and mobile clients for the frank talk community platform.",
...
async function handleStartManagedAgent(
  ...
  if (isRelayMeshManagedAgent(agent)) {
    ...
    if (!hasLiveTarget) {
      throw new Error(
        "frank talk shared compute cannot start because no live member is serving this model.",
      );
    }
  }
        ...
        return {
          ...
          files: [
            ...
            },
            {
              ...
              size: 33120,
              preview_content:
                "// Smart HTTP git transport\n// Handles upload-pack and receive-pack for frank talk git repos.\n",
            },
          ],
        };
      ...
      case "discover_agent_models": {
        ...
        if (provider === "relay-mesh") {
          ...
          if (mockMeshState.models.length === 0) {
            throw new Error(
              "no frank talk shared compute serving members are available",
            );
          }
        }
        const models =
